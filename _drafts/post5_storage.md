...
Because these supply levels are close to peak consumption, with consumption only exceeding these levels for at most a few hours a day in the summer, the hypothetical battery storage is for the most part able to fully recharge every day (like the 135% solar example above).  Thus the maximum demand on storage is simply how far above the supply level the consumption gets on each day, for how many hours:
...
<tr class="blue">
 ...
 <tr class="coral">
  <th scope="col" style="background-color: #E8E8E8">Average Summer Peak</th>
   <td>64,115</td> <td>7.30</td> <td>163%</td> <td>7.30</td> <td>82%</td> <td>18</td> <td>7</td> <td><b>11.4 GWh</b></td></tr>
<tr class="plum">
  <th scope="col" style="background-color: #E8E8E8">Year's 95th %ile Peak</th>
  <td>73,224</td>  <td>8.34</td> <td>186%</td> <td>8.34</td> <td>93%</td> <td>68</td> <td>0</td> <td><b>2.6 GWh</b></td></tr>
</table>
...
The average-summer-peak architecture sees a handful of days where the total daily generation does not cover total demand, so storage usage accumulates day over day.  But because these days are so few, the accumulation is modest, and only 12 GWh of storage are needed. 
...
The 135% generation profile also carries some lessons.   Like the 150% case, we still have the daily cycles to the tune of 50 - 75 GWh per day.  But with the lower generation level we once again have a number of days where the total usage exceeds total supply.   However, this problem does not occur in the summer: increased solar production covers that increase in demand.  Instead it happens in winter where solar electricity supply falls disproportionately more than electricity consumption.  This phenenomenon will be exacerbated in climates where winter electricity consumption **increases** (e.g., for heating) while solar generation falls.  

One subtle point is that the seasonality of solar electricity supply does not quite match the seasonality of electricity demand.   Temperature lags the solar calendar by about a month because the atmosphere takes some time to warm up and cool down after solar energy input changes.   This is why in the northern hemisphere the longest daylight is in June but the hottest weather is generally in July.  Since HVAC electricity usage follows temperature, notice that peak consumption occurs in July through September, when the solar output has begun to fall from early summer peaks.   So solar over-capacity needs to be higher than it would if there was perfect seasonal alignment. 
